...
void initASL(void){
    ...
    for(int i=0; i<MAXPROC; i++){ //inizializza ed aggiunge MAXPROC semafori liberi alla semdFree
...
int insertBlocked(int *key, pcb_t* p){
    ...
    else
    {
        if(list_empty(semdFree_h)) { 
            ...
            struct semd_t *sem = container_of(semdFree_h->next,semd_t,s_next); //crea un nuovo semaforo se non esiste uno con la key associata 
            list_del(semdFree_h->next);
            list_add(&(sem->s_next),semd_h);

            insertProcQ(&(sem->s_procQ), p); //poi aggiunge il processo nella procQ 
            sem->s_key = key;
